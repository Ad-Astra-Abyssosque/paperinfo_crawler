...
可借助大模型阅读json文件，快速筛选特定主题的论文。

**参考提示词：**

> 给出的json文件中包含CCS 2024年发表的所有文章信息。json文件内容是一个数组，数组的每个元素对应一篇文章的信息，每个元素含有title、url、abstract三个键值对。
>
> 请你根据这些信息筛选有关“C2检测与对抗”，“恶意二进制检测与对抗”、“EDR绕过与对抗”，“入侵检测”的文章，并按相关性筛选输出一个简表，使用中文回答。
>
> 关键词可以包括但不限于“C2”, “EDR”，“beacon detection”, “SIEM rules”，“恶意进程检测”，“异常行为检测”，“主机侧攻击”，“入侵检测”等

**建议：**

- 每次只问一个文件，如果文件太大可以考虑拆分

- 每个文件新开一个对话，否则大模型容易把之前文件的内容也混进去
